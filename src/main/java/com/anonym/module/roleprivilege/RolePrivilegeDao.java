package com.anonym.module.roleprivilege;

import com.anonym.module.privilege.domain.PrivilegeEntity;
import com.anonym.module.roleprivilege.domain.RolePrivilegeEntity;
import com.baomidou.mybatisplus.mapper.BaseMapper;
import org.apache.ibatis.annotations.Mapper;
import org.apache.ibatis.annotations.Param;
import org.springframework.stereotype.Component;

import java.util.List;


@Mapper
@Component
public interface RolePrivilegeDao extends BaseMapper<RolePrivilegeEntity> {

    /**
     * 根据角色id删除
     *
     * @param roleId
     */
    void deleteByRoleId(@Param("roleId") Integer roleId);

    /**
     * 删除权限所关联的角色信息
     *
     * @param privilegeIds
     */
    void deleteByPrivilegeIds(@Param("privilegeIds") List<Integer> privilegeIds);

    /**
     * 批量添加
     *
     * @param rolePrivilegeList
     */
    void batchInsert(List<RolePrivilegeEntity> rolePrivilegeList);

    /**
     * 查询某批角色的权限
     *
     * @param roleIds
     * @return
     */
    List<PrivilegeEntity> listByRoleIds(@Param("roleIds") List<Integer> roleIds, @Param("normalStatus") Integer normalStatus);

    /**
     * 查询某个角色的权限
     *
     * @param roleId
     * @return
     */
    List<PrivilegeEntity> listByRoleId(@Param("roleId") Integer roleId);
}
